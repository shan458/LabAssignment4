...
We first set max_tries = 3. Then we create a loop to iterate three times. For each iteration, we ask the user for the answer (user input). Then based on the answer the user gives, we check to see if the user input matches the answer. If so, print "Correct!", then terminate the loop with a break statement. 
...
def ask(question, answer, max_tries=3):
    ...
    while tries < max_tries:
        ...
        if ans == answer:
            ...
            break
    if ans != answer:
        print("You have used up your main allotment of guesses.")
